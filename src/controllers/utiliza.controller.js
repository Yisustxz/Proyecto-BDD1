...
const addUtiliza = async (req, res) => {
  try {
    ...
    const productoUsado = await pool.query({
      text: "SELECT * FROM productos WHERE cod_producto = $1",
      values: [newUtiliza[3]],
    });

    const precioActual = productoUsado.rows[0].precio;
    newUtiliza.push(precioActual);

    const insertar = await pool.query({
      text: "INSERT INTO utiliza (num_unico, num_detalle, ci_trabajador, cod_producto, cantidad_usada, precio_actual) VALUES ($1, $2, $3, $4, $5, $6) RETURNING num_unico, num_detalle, ci_trabajador, cod_producto",
      values: newUtiliza,
    });
